...
def build():
    ...
    set_run_font(version.add_run("版本 1.3  |  真实到手价与优惠工作流版"), size=11, color=NAVY, bold=True)
    ...
    for index, item in enumerate((
        "产品简介与快速开始", "主界面与经营驾驶舱", "价格采集中心", "优惠策略中心", "数据导入",
    ...
    add_body(doc, "价策 AI 是面向电商商家的价格情报与定价决策产品。系统基于用户本机授权账号采集商品与优惠信息，在购物车和提交订单前页面复核结算预览价，再结合商品价格、成本、销量、库存与商品角色，完成价格诊断、建议区间、调价模拟和可视化报告。")
    ...
    add_steps(doc, [
        ...
        "如需接入本地采集连接层，另开终端执行 pnpm collector:serve；真实平台需配置对应浏览器适配器。",
        "如需优惠链接识别与优惠组合核算，再开一个终端执行 python -m collector_python.api。",
    ])

    new_content_page(doc)
    add_heading(doc, "3. 主界面与经营驾驶舱", 1)
    add_figure(doc, "dashboard.png", "图 1  经营驾驶舱：全店价格健康度和优先调价机会")
    add_body(doc, "左侧导航包含经营驾驶舱、价格采集中心、优惠策略中心、价格诊断、价格拐点分析、调价模拟器、报告中心、数据中心和使用指引九个模块。")
    add_bullets(doc, [
    ...
    add_body(doc, "采集中心将面试演示与登录态采集明确隔离。真实任务必须先为各平台选择已授权账号，再输出结算预览价、账号画像、验证方式、采集时间和失败原因。")
    add_heading(doc, "4.1 本地采集步骤", 2)
    add_steps(doc, [
        "在项目目录运行 pnpm collector:serve。",
        "进入价格采集中心，为每个平台选择已授权账号。",
        "粘贴账号可访问的商品页链接，每行一个，然后点击“立即采集”。",
    ...
    add_bullets(doc, [
        ...
        "识别优惠、核算购物车并进入提交订单前页面二次复核。",
        "不绕过登录、验证码或访问控制；商业部署优先使用平台开放 API。",
        "密钥、Cookie 和账号信息不得提交到 GitHub 或下发浏览器。",
    ])
    add_heading(doc, "4.3 按品牌选择商品", 2)
    add_steps(doc, [
        "在价格采集中心切换到“按品牌选品”。",
        "按品牌、系列、规格或关键词筛选商家货盘。",
        "勾选目标 SKU，也可选择当前筛选结果中的全部商品。",
        "多选京东、天猫、拼多多等目标平台，并确认各平台采集账号。",
        "生成“SKU × 平台”采集任务；低置信度同款进入人工复核。",
    ])
    add_body(doc, "系统以商家在售货盘为选品基准，不直接把品牌搜索结果全部作为同款，从而避免混入不同段位、规格、组合装和赠品装。")

    new_content_page(doc)
    add_heading(doc, "5. 优惠策略中心", 1)
    add_body(doc, "优惠策略中心统一展示账号结算预览价，核算平台券、店铺券、品类券、会员折扣、满减、补贴和运费。每个平台支持多个账号独立保存和切换，报告显示账号别名、会员等级、收货地区与优惠范围。")
    add_heading(doc, "5.1 一次登录与三档领取模式", 2)
    add_bullets(doc, [
        "用户在平台官方页面完成首次登录，后续在本机复用授权状态；系统不收集账号密码。",
        "仅识别：识别并计算优惠，不触发领取。",
        "逐次确认（推荐）：领取前展示金额、门槛、有效期与限制，由用户确认。",
        "规则授权：只领取免费、无需实名且确实降低目标商品价格的优惠。",
        "遇到验证码、实名、付费会员、支付或风险提示时立即暂停并交还用户。",
    ])
    add_heading(doc, "5.2 商家优惠链接与价格核验", 2)
    add_steps(doc, [
        "粘贴商家提供的官方优惠链接。",
        "检查 HTTPS、平台域名；短链跳转后再次校验最终域名。",
        "读取活动标识、门槛、有效期、适用 SKU 和可叠加关系。",
        "枚举合法优惠组合，计算最低可实现价格。",
        "结算前二次验证；失败时降级为估算价，不标记为实际价。",
    ])
    add_table(doc, ["价格口径", "定义", "报告要求"], [
        ("账号结算预览价", "指定账号、地区、数量和优惠下，提交订单前页面的应付金额", "显示账号画像与条件"),
        ("已支付成交价", "用户实际完成支付的订单金额", "仅订单回传后使用"),
    ], [2100, 4500, 2760])

    new_content_page(doc)
    add_heading(doc, "6. 数据导入", 1)
    add_body(doc, "进入“数据中心”，点击“下载导入模板”获取标准 CSV 模板。填写后点击“选择 CSV 文件”完成导入。")
    add_heading(doc, "6.1 必填字段", 2)
    add_table(doc, ["字段", "用途", "示例"], [
        ("商品编码", "SKU 唯一标识", "P001"), ("品类", "品类筛选与分组", "食品饮料"),
        ("品牌", "商品标准化", "伊利"),
    ...
    add_heading(doc, "11.1 多品类头部 SKU 跨平台价格矩阵", 2)
    add_steps(doc, [
        "在报告中心切换到品牌跨平台比价，选择品类与品牌。",
        "选择 Top 3、Top 5 或 Top 10；系统按近 30 天销量确定头部 SKU。",
        "横向查看价格矩阵：一行一个标准 SKU，一个平台一列。",
        "查看价格热力、平台价格指数、SKU 价差率和价格结论。",
        "点击“导出矩阵”下载 CSV。",
    ])
    add_body(doc, "品牌相同不代表商品同款。报告仍按系列、段位与规格逐 SKU 比较；平台作为列展示所选账号的结算预览价，并在报告顶部明确账号条件。切换账号后生成新的报告版本。")

    new_content_page(doc)
    add_heading(doc, "12. 建议操作流程", 1)
    add_steps(doc, [
        "确认各平台授权账号、会员等级、收货地区和会话有效期。",
    ...
    new_content_page(doc)
    add_heading(doc, "13. 数据安全与版本边界", 1)
    add_bullets(doc, [
        "当前数据保存在浏览器 localStorage 中，清除浏览器数据可能导致导入内容丢失。",
        "本地采集服务只监听 127.0.0.1，不对公网开放。",
        "系统不接收账号密码；授权状态文件只保存在用户本机，也不会自动执行调价。",
        "遇到验证码、风控、付费会员或支付步骤时暂停并交还用户。",
        "演示竞品价格不代表真实市场报价。",
    ...
    for question, answer in (
        ("登录态采集失败", "检查所选账号会话是否有效；遇到验证码或风控时需用户在官方页面重新授权。"),
    ...
    add_heading(doc, "15. 后续商用升级建议", 1)
    add_bullets(doc, [
        "接入真实平台授权 API、授权续期和调用监控。",
        "接入用户授权浏览器、优惠领取后核验和异常价格复核。",
        "增加定时任务、历史报价数据库与异常告警。",
        "增加多商家账号、权限与数据隔离。",
        "增加商品匹配人工审核工作台。",
        "保存调价审批、执行与实验记录。",
