...
def build():
    ...
    add_body(doc, "价策 AI 是面向电商商家的价格情报与定价决策产品。系统从平台开放 API、合规公开商品页、用户本机授权态或 CSV 获得公开价与优惠条件，分层核算真实到手价，再结合商品价格、成本、销量、库存与商品角色，完成价格诊断、建议区间、调价模拟和可视化报告。演示货盘覆盖母婴奶粉、食品饮料、家用电器和个护清洁，导入 CSV 可继续扩展品类。")
    ...
    add_body(doc, "优惠策略中心将划线价、公开售价、条件可实现价和已持券实际价分层展示，核算平台券、店铺券、品类券、会员折扣、满减、补贴和运费。会员、新客、地区及账户定向价格不会混入公开市场中位数。")
    ...
    add_table(doc, ["价格口径", "定义", "市场比较"], [
        ("划线价", "页面参考原价", "不进入"),
        ("公开售价", "无需登录或领券即可复现", "进入公开中位数"),
        ("条件可实现价", "满足门槛并可领取优惠", "仅同条件比较"),
        ("已持券实际价", "当前授权账户可验证价格", "单独展示"),
    ], [1900, 4700, 2760])
    ...
    add_body(doc, "品牌相同不代表商品同款。报告仍按系列、段位与规格逐 SKU 比较，账户专属优惠单独展示，不混入公开平台价格矩阵。")
